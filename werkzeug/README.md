...
| `node blattzu.js` | Ghost-Click-Schutz der Blätter: Scrim schließt auf pointerdown, der Folge-Klick desselben Tipps wird einmalig verschluckt — erneuter Tipp auf `#monthBtn`/`#weekLabel` toggelt sauber zu, ein Tipp daneben öffnet nichts Fremdes, Escape bleibt schluckerfrei, normaler Gebrauch nach 400 ms unverändert. Läuft mit **echten Touch-Events** auf dem iPhone-15-Pro-Profil — mit Maus-Klicks reproduziert die Fehlerklasse nicht |
| `node streifenwisch.js` | Der wischbare Tagesstreifen (echte CDP-Touch-Gesten, 393×852): Wisch wechselt den Tag inkl. nahtlosem Wochenübergang (`anchor`+Idx+Label in einem Zug), Tap bleibt Tap (TAP_SLOP), Pfeile funktionieren weiter, vertikales Scrollen wird nicht gekapert; Formatwerte ab 360/390 px (Pfeile 34 px, Lücken 3 px, ✓/× sichtbar 40 px, `--hourh` 60) und der Beweis, dass das 320-px-SE-Format byte-identisch bleibt |
...
| `node zielfrage.js` | Die verschmolzene Wann-Frage der Ziele-Karte: genau eine Mo–So-Reihe und ein Zeitpaar, Saat = Schnittmenge aus `plan.days`/`from`/`to` und Alt-`fenster`, Speichern schreibt das Trio und räumt das Fenster (Anker bleibt), unverändertes Speichern ändert die Verteilung nicht, ein zeitlich unvereinbares Alt-Fenster wird als Konfliktzeile benannt (nie stillschweigend aufgeweitet — `p.to` fällt nie auf null), der Einschränkungs-Chip liest das Trio, die Einmal-Umlegung gefensterter Bereiche wird als Summe+Fenstertreue gemessen, das Aufgaben-Blatt behält seinen Fenster-Block |
...
| `node serie.js` | Zweiwöchentliche Termine: Parität zur `since`-Woche über die Sommerzeitumstellung hinweg, eigener Haken-Schlüssel je Woche, Serien-Rückfrage bei Löschen/Verschieben. Abschnitt h: Einzel-Ausnahmen (`b.ausnahmen`) — Drei-Wege-Rückfrage „Nur diesen Termin", nur das eine Datum verschwindet (Parität der übrigen unverändert), `migrate()`-Idempotenz, mergeStates-Duell (neueres `at` gewinnt), der verwaiste `erledigt`-Schlüssel bleibt liegen, `freeGaps()` gibt den Slot frei, „Ganze Serie" löscht weiter mit Grabstein |
