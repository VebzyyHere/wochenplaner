...
Tastaturweg, und das Leisten-Label springt zum frühesten Vorschlag), `monat.js` (die
...
**Zustand.** Ein einziges `state`-Objekt. `freshState()` (2188) legt es bei `version: 8` an,
`migrate()` (2219) läuft danach sofort und zieht jeden Stand — auch einen frischen — kumulativ auf
...
(Prüfung: `grobstandard.js`). `Store` (1920) schreibt nach
...
`Store.backupVorV9()` (1982) sichert **einmalig** den unveränderten Stand, bevor `migrate()` ihn
...
**Speichern.** `save()` (2497) → `stampChanges()` → `Store.save()` → `syncPush()`.
`snapshot()`/`recHash()` (2457/2451) vergleichen den neuen Stand mit dem letzten: was sich geändert
...
aussprechbaren Zeitfenstern — gespeist aus `freeGaps()` (3888), derselben Lückenberechnung wie
beim Verteiler; `FREIZEIT_MIN = 30` (2778) blendet Kurzlücken aus. **„Belegt"** zeigt dieselbe
...
**Monatsübersicht.** `monatSheet(startDatum)` (3202, mit `monatMontage()` 3168 und
`wochenBelegung()` 3187), hinter `#monthBtn` in der Topbar: Kalenderraster mit **KW-Rinne** —
...
das Wochen-Blatt, künftige Wochen tragen „+" und planen über `planeWoche(montag)` (4820, durchs
Gate — schloss die letzte Gate-Lücke von `planeNaechsteWoche()` 4835, heute ein Einzeiler).
...
**Verteiler.** `buildSuggestions()` (4477) → `placeArea()` / `placeGrob()` / `growSuggestions()`.
...
Monats-„+" — laufen durch `verteilenMitGate()` (4845): bei `ok: false` erscheint „Das wird eng"
mit dem Ausweg „Nächste Woche planen" (`planeNaechsteWoche()` 4835, seit der M-Runde ein
Einzeiler über `planeWoche()` 4820; Prüfung: `kapazitaet.js`, für den Monats-Weg `monat.js`).
...
`doppeltipp.js`).
`VERPLANT_GRENZE = 0.65` (3738), Ampel
grün ≤ 60 %, gelb ≤ 70 %, darüber rot (`ampelFarbe()` 3805). Begründungen: der generische
Fallback (`GRUND_GENERISCH` 4176) erscheint auf Blöcken und Agenda-Zeilen **nicht** mehr
(`grundZumZeigen()` 4202) — nur die Blätter (`sugSheet`/`blockSheet`) zeigen ihn weiterhin;
das Datenfeld `b.grund` bleibt immer gesetzt (Prüfung: `erklaer.js`). `istSerie()` (3390,
`repeat === "weekly" || "2wochen"`) vereinheitlicht wöchentliche und zweiwöchentliche Termine
für Anzeige und Abhaken.

**Rest des Tages.** `restDesTagesBauen()` (4663), aufrufbar über den Knopf in „Heute"
(Sichtbarkeit über `restDesTagesMoeglich()` 4709), wendet denselben Verteiler wie das
...
- **`freeGaps()` und `tagesAuslastung()` widersprechen sich bei groben Blöcken — beide zu Recht,
  nicht anfassen.** `freeGaps()` (3888) blendet grobe Blöcke aus, weil sie keine Uhrzeit haben, die
  es aussparen könnte; `tagesAuslastung()` (3087) zählt dieselben Blöcke trotzdem mit, weil ihre
  Dauer real verplante Zeit ist, nur ihre Uhrzeit nicht. Das Frei-Gesicht des Wochen-Blatts
  (`freizeitSheet()`, 2864) macht diesen Unterschied absichtlich sichtbar, statt ihn aufzulösen: hinter den echten
...
- **Abhaken hängt am Paar Eintrag + Datum** (`hakenKey()` 3536, nutzt `istSerie()` 3390), nicht an
...
- **`growSuggestions()` kennt seit Stufe 16 „jetzt".** War bekannt, bewusst nicht behoben, solange
  ...
  aber auch den wöchentlichen Verteiler, weil `clearSuggestions(warm)` (4542) den laufenden Tag
